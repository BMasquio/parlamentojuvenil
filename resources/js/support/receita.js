...
function ValidarDados() {
    ...
    if (
        pegaObj('txtDataNascimento').value.length != 10 ||
        pegaObj('txtDataNascimento').value == '00/00/0000'
    ) {
        alert(
            'Informe a data de nascimento do titular do CPF a ser consultado, com dois dígitos para o DIA e para o MÊS e quatro dígitos para o ANO. Formato: dd/mm/aaaa.',
        )
...
function Consultar() {
    ...
    $('#temptxtToken_captcha_serpro_gov_br').val(
        $("input[name*='Token_captcha_serpro_gov_br']").val(),
    )
    $('#temptxtTexto_captcha_serpro_gov_br').val(
        $("input[name*='txtTexto_captcha_serpro_gov_br']").val(),
    )
